...
{-@ turn :: x:Int -> {worry:Int | worry /= 0 } ->
   {modulus:Int | modulus /= 0} -> m:MonkeyIR x -> {d:[MonkeyItem x] | len d = len (items m)} @-}
...
data MonkeyList =
  Empty |
  MCons { headMonkey :: MonkeyIR 8, barrel :: MonkeyList }
@-}
data MonkeyList =
  Empty |
...
{-@ roundN :: x:Int -> {worry:Int | worry /= 0 } -> {modulus:Int | modulus /= 0} -> {before:[MonkeyIR x] | len before = x} -> {n:Nat | n < x }
...
roundN x worry modulus monkeys n =
  let m = (monkeys !! n)
      ...
      newCount = (count m) + length (items m)
...
{-@ round :: x:Int -> {worry:Int | worry /= 0} -> {modulus:Int | modulus /= 0 } -> {before:[MonkeyIR x] | len before = x}
    -> {after:[MonkeyIR x] | len after = x} @-}
round :: Int -> Int -> Int -> [Monkey] -> [Monkey]
round x worry modulus monkeys = go 0 monkeys where
  {-@ go :: {n:Int | n >= 0 && n <= x} -> {m:[MonkeyIR x] | len m = x} -> {m2:[MonkeyIR x] | len m2 = x}
      / [ x - n ] @-}
...
part1 _ = do
  ...
  let m1 = computeModulus example in do
    ...
    let allRounds = iterate (Main.round 4 3 m1) example in
      print $ allRounds !! 20
    
  let m2 = computeModulus input in do
    putStrLn $ "Working mod " ++ show m2    
    let allRounds2 = iterate (Main.round 8 3 m2) input in
      print $ allRounds2 !! 20
...
part2 _ = do
  ...
  let m1 = computeModulus example in do
    putStrLn $ "Working mod " ++ show m1
    let allRounds = iterate (Main.round 4 1 m1) example in do
  ...
  let m2 = computeModulus input in do
    putStrLn $ "Working mod " ++ show m2    
    let allRounds2 = iterate (Main.round 8 1 m2) input in
